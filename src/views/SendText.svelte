<script type="ts">
    ...
    function appendMessage(message: string, source: string) {
        if (source === 'user' && !appSettings.echoLocal) {
            return;
        }
        const prepend = appSettings.showTimestamp ? `[${timeString(true)}]` : `&gt;`;
...
<div class="window-body">
    <fieldset>
        ...
        <br/>
        <div class="form-group">
            <input type="checkbox" id="local-echo" bind:checked={appSettings.echoLocal}/>
            <label for="local-echo">Echo Input</label>
            &nbsp;
            <input type="checkbox" id="timestamp" bind:checked={appSettings.showTimestamp}/>
            <label for="timestamp">Show Timestamp</label>
            &nbsp;
            <input type="checkbox" id="auto-scroll" bind:checked={appSettings.autoScroll}/>
            <label for="auto-scroll">Auto Scroll</label>
            &nbsp;
            <input type="checkbox" id="keyboards" bind:checked={appSettings.showKeyboards}/>
            <label for="keyboards">Show Virtual Keyboards</label>
        </div>
    </fieldset>

    {#if appSettings.showKeyboards}
        <TabContainer>
            <TabMenu>
