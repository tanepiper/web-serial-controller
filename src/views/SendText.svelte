<script type='ts'>
  ...
  function appendMessage(message: string, source: string) {
    ...
    if (source === 'user') {
      ...
      const now = new Date();
      const time = [now.getHours(), now.getMinutes(), now.getSeconds()].map(value => value < 10 ? `0${value}` : `${value}`).join(':');
      prepend = `[${time}]`;
    }
...
<pre class='text-output' bind:this={outputEl}></pre>
